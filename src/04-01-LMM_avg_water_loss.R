...
summary(lmm_avg_loss_WW)
summary(lmm_avg_loss_WD)
...
rsquared(lmm_avg_loss_WD)
rsquared(lmm_avg_loss_WW)
...
# avg ret 
vif.mer(lmm_avg_loss_WD)
vif.mer(lmm_avg_loss_WW)
...
plot(lmm_avg_loss_WD)
plot(lmm_avg_loss_WW)

# diagnostic plots: histogram of residuals
# checking assumption of normally disributed residuals
hist(residuals(lmm_avg_loss_WD))
hist(residuals(lmm_avg_loss_WW))

# diagnostic plots: leverage
# checking assumptions regarding sensitivity to data (esp outliers)
lev_pearson_WD <- data.frame(lev = hatvalues(lmm_avg_loss_WD), 
                             pearson = residuals(lmm_avg_loss_WD, 
                                                 type = "pearson"))
...
lev_pearson_WW <- data.frame(lev = hatvalues(lmm_avg_loss_WW), 
                             pearson = residuals(lmm_avg_loss_WW, 
                                                 type = "pearson"))
...
r1Var_WD <- as.numeric(VarCorr(lmm_avg_loss_WD)[["g1"]])
residVar_WD <- attr(VarCorr(lmm_avg_loss_WD), "sc")^2
r1Var_WD
residVar_WD
